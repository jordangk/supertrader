...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4) {
    ...
    async function tick() {
      ...
      // MAX POSITION CHECK: query ACTUAL Kalshi positions directly — not just DB
      try {
        let ksShares = 0;
        // Query Kalshi portfolio for this specific market
        if (sess.kalshiTicker) {
          const posR = await kalshiFetch(`${KALSHI_TRADE_API}/portfolio/positions?ticker=${encodeURIComponent(sess.kalshiTicker)}&limit=1`);
          if (posR.ok) {
            const posD = await posR.json();
            const mp = posD.market_positions?.[0] || posD.positions?.[0];
            ksShares = Math.abs(parseInt(mp?.total_traded || mp?.position || '0'));
          }
        }
        // Also check DB for Poly fills
        ...
        if (!state.lastCapLog2 || Date.now() - state.lastCapLog2 > 30000) {
          ...
          if (ksShares > 0 || polyShares > 0) console.log(`[maker:${campaignId.slice(0,8)}] Positions: KS ${ksShares}/30 Poly ${polyShares}/30`);
        }
