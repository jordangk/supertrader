  ...
  app.get('/api/arb/event-pnl', async (req, res) => {
    try {
      ...
      for (const [ticker, d] of Object.entries(ks)) {
        ...
        // KS P&L = payout - total spent (cost + fees)
        const ksPnl = d.payout - d.cost - d.fees;
        const ksDetail = [];
        for (const o of d.orders) {
          const won = (o.side === result);
          ksDetail.push({ side: o.side, shares: o.shares, cost: o.cost + o.fee, won });
        }
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4, maxShares = GLOBAL_MAX_SHARES, polyOnlyInput = false) {
    ...
    async function tick() {
      ...
      if (sess.polyOnlyInput) {
        ...
        if (state.nextStrategy === 'A' || state.nextStrategy === 'B') {
        ...
        try {
          ...
          if (!er.ok) {
            ...
            const ed = await er.json().catch(() => ({}));
            if (ed?.success) state.nextStrategy = bestStrategy === 'A' ? 'B' : 'A';
          }
        } catch {}
        return;
      }
      ...
      try {
        ...
        if (!clob) {
          ...
          if (!polyFilled) {
            ...
            if (ksFilled > 0) state.nextStrategy = bestStrategy === 'A' ? 'B' : 'A';
