...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  async function placeKalshiOrder(ticker, side, count, priceCents, opts = {}) {
    ...
    if (priceCents == null || priceCents < 1 || priceCents > 99) {
      throw new Error(`Kalshi limit invalid: ${priceCents}¢ (need 1–99)`);
    }
    const body = {
      ticker,
      action: 'buy',
    ...
    async function tick() {
      ...
      try {
        ...
        if (ksShares >= 30 || polyShares >= 30) {
          ...
          logSkipIfGoodEdge('position_cap', `KS=${ksShares} Poly=${polyShares}`);
          return;
        }
      } catch (capErr) {
      ...
      console.log(`[arb:${campaignId.slice(0,8)}] PARALLEL: KS ${ksSide.toUpperCase()} ${SHARES}sh @ ${ksAskPrice}¢ + Poly ${SHARES}sh @ 99¢ (mkt ask ${(polyAsk * 100).toFixed(0)}¢) = ${ksAskPrice + 99}¢ profit:${(entryProfit * 100).toFixed(1)}¢`);
      ...
      try {
        ...
        if (!clob) {
          ...
          const [kalshiResult, polyResult] = await Promise.allSettled([
            placeKalshiOrder(sess.kalshiTicker, ksSide, SHARES, ksAskPrice),
            (async () => {
              const signed = await clob.createOrder({
                tokenID: polyTokenId,
                price: polyPrice,
          ...
          const kalshiMakerCost = kalshiOrder ? parseFloat(kalshiOrder.maker_fill_cost_dollars || '0') : 0;
          const kalshiFillCost = kalshiTakerCost + kalshiMakerCost;
          const kalshiRealPrice = ksFilled > 0 ? kalshiFillCost / ksFilled : ksAskPrice / 100;
          const kalshiFees = parseFloat(kalshiOrder?.taker_fees_dollars || '0') + parseFloat(kalshiOrder?.maker_fees_dollars || '0');
