...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4) {
    ...
    async function tick() {
          ...
          if (filled >= 5 || status === 'executed') {
            ...
            if (clob) {
              try {
                ...
                const result = await clob.postOrder(signed, 'FOK'); // FOK ensures exact fill or nothing
      ...
      // MAX POSITION CHECK: 50 shares per event on EACH platform
      try {
        const posCheck = await pool.query(
          `SELECT COALESCE(SUM(CASE WHEN kalshi_filled THEN kalshi_shares ELSE 0 END), 0) as ks_shares,
                  COALESCE(SUM(CASE WHEN poly_filled THEN poly_shares ELSE 0 END), 0) as poly_shares
           FROM arb_trades WHERE session_id = $1`,
          [state.currentSessionId],
        );
        const ksShares = parseInt(posCheck.rows[0]?.ks_shares || '0');
        const polyShares = parseInt(posCheck.rows[0]?.poly_shares || '0');
        if (ksShares >= 50 || polyShares >= 50) {
          ...
          // Disable this campaign in DB
          await pool.query('UPDATE arb_campaigns SET auto_enabled = false, status = $1, stopped_at = NOW() WHERE id = $2', ['stopped', campaignId]).catch(() => {});
          stopped = true; // kill the loop
          console.log(`[maker:${campaignId.slice(0,8)}] CAP HIT: KS ${ksShares}/30 Poly ${polyShares}/30 — DISABLED campaign, cancelled all orders`);
          return;
        }
      } catch {}
