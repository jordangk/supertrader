          ...
          if (filledShares <= 2) {
            ...
            if (clob) {
              try {
                ...
                console.log(`[arb:${campaignId.slice(0,8)}] Poly: ${result?.status} ${polyActualShares.toFixed(1)}sh @ ${(polyActualPrice*100).toFixed(0)}¢`);

                // Update DB with Poly result
                await pool.query(
                  `UPDATE arb_trades SET poly_filled = $1, poly_fill_price = $2, poly_shares = 5, poly_order_id = $3, poly_actual_shares = $4, both_filled = $1 WHERE kalshi_order_id = $5`,
                  [polyFilled, polyActualPrice, result?.orderID, polyActualShares, ksOrderId],
                ).catch(() => {});
                ...
                console.error(`[arb:${campaignId.slice(0,8)}] Poly error:`, e.message?.slice(0, 100));
              }
            }
          }
        }
