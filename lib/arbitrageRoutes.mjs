...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4) {
    ...
    const state = {
      ...
      pendingPolyTokenId: null, // which poly token to FAK when filled
    ...
    const iv = setInterval(async () => {
      ...
      if (recurring && recurringType && !state.renewLock) {
        ...
        if (urls && urls.slot !== state.currentSlot) {
          ...
          // Cancel any pending order before renewing
      ...
      // ── MAKER STRATEGY: place KS limit, wait for fill, then FAK Poly ──
      ...
      if (state.pendingOrderId) {
        try {
          ...
          // FILLED → FAK Poly immediately
          if (filled >= 5 || status === 'executed') {
            const filledShares = Math.max(filled, 5);
            console.log(`[maker:${campaignId.slice(0,8)}] KS FILLED ${filled}sh @ ${state.pendingKsPrice}¢ → FAK Poly`);
            ...
            if (clob) {
              try {
                ...
                const signed = await clob.createOrder({
                  tokenID: state.pendingPolyTokenId,
                  price: Math.min(0.99, state.pendingPolyPrice + 0.15), // +15¢ buffer to fill
                  size: filledShares,
                  side: 'BUY',
                }, { tickSize: tickSize || '0.01', negRisk: negRisk || false });
                const result = await clob.postOrder(signed, 'FOK'); // Fill-or-Kill
                const polyFilled = result?.status === 'matched';
                console.log(`[maker:${campaignId.slice(0,8)}] Poly FOK: ${result?.status} taking=${result?.takingAmount || '?'}`);

                // Log to DB
                const polySide = state.pendingStrategy === 'A' ? 'down' : 'up';
                const polyActualCost = result?.makingAmount && result?.takingAmount ? parseFloat(result.makingAmount) / parseFloat(result.takingAmount) : state.pendingPolyPrice;
                await pool.query(
                  ...
                  [state.currentSessionId, state.pendingStrategy, sess.kalshiTicker, state.pendingKsSide, state.pendingKsPrice,
                   true, state.pendingKsPrice / 100, 5, state.pendingOrderId,
                   state.pendingPolyTokenId, polySide, state.pendingPolyPrice,
                   polyFilled, polyActualCost, 5, result?.orderID || null,
                   polyFilled, (state.pendingKsPrice / 100 + polyActualCost) * 5, 5, 0],
            ...
            // Cooldown logged in DB — next check will see it
            return;
          }
          ...
          // STILL RESTING → check if spread is still good
          const currentPolyAsk = state.pendingStrategy === 'A' ? pDownAsk : pUpAsk;
          const currentKsAsk = state.pendingKsSide === 'yes' ? ksYesAsk : ksNoAsk;
          if (currentPolyAsk != null && currentKsAsk != null) {
            const currentCost = currentKsAsk + currentPolyAsk;
            const currentProfit = 1 - fee - currentCost;
            if (currentProfit < halfTarget) {
              // Spread gone — cancel
              try { await kalshiFetch(`${KALSHI_TRADE_API}/portfolio/orders/${state.pendingOrderId}`, { method: 'DELETE' }); } catch {}
              console.log(`[maker:${campaignId.slice(0,8)}] Cancelled — spread gone (${(currentProfit*100).toFixed(1)}¢ < ${(halfTarget*100).toFixed(0)}¢)`);
              state.pendingOrderId = null;
            }
          }
          // Partial fill stays active — don't place another
          return;
        } catch (e) {
      ...
      if (!state.lastDebug || Date.now() - state.lastDebug > 30000) {
        ...
        console.log(`[maker:${campaignId.slice(0,8)}] Book: A=${profitA != null ? (profitA*100).toFixed(1)+'¢' : '—'} B=${profitB != null ? (profitB*100).toFixed(1)+'¢' : '—'} | threshold=${thresholdCents}¢ | no pending`);
      }
      ...
      if (bestStrategy === 'A') {
        ksSide = 'yes';
        // Maker: bid at current ask - 2¢ (sit on the book)
        ksLimitPrice = Math.max(1, Math.round((ksYesAsk - 0.02) * 100));
        polyTokenId = polyTokens?.[polyDownIdx];
        polyAsk = pDownAsk;
      } else {
        ksSide = 'no';
        ksLimitPrice = Math.max(1, Math.round((ksNoAsk - 0.02) * 100));
        polyTokenId = polyTokens?.[polyUpIdx];
        polyAsk = pUpAsk;
      }

      // Verify combined cost < 96¢ with our maker price
      const makerCost = ksLimitPrice / 100 + polyAsk;
      if (makerCost > 0.96) return;

      // Check book depth (50+ shares)
      ...
      if (ksAskQty != null && ksAskQty < 50) return;
      if (polyAskQty != null && polyAskQty < 50) return;

      // Place the maker limit order on Kalshi
      state.lock = true;
      try {
        console.log(`[maker:${campaignId.slice(0,8)}] Placing KS ${ksSide.toUpperCase()} limit @ ${ksLimitPrice}¢ (ask ${ksSide === 'yes' ? Math.round(ksYesAsk*100) : Math.round(ksNoAsk*100)}¢ - 2¢)`);
        ...
        if (res.ok && data.order?.order_id) {
          ...
          // Check if it filled immediately (taker match)
