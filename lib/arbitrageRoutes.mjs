...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  app.post('/api/arb/execute', async (req, res) => {
    try {
      ...
      console.log(`[arb-execute] Strategy ${strategy}: Kalshi ${kalshiSide.toUpperCase()} ${SHARES}@${kalshiPriceCents}¢ + Poly ${strategy === 'A' ? 'Down' : 'Up'} ${SHARES}@${(polyPrice * 100).toFixed(0)}¢`);
  ...
  app.post('/api/arb/campaigns', async (req, res) => {
    try {
      const { kalshiUrl, polyUrl, label, autoEnabled, autoThreshold, autoCooldown, swapPoly, recurring } = req.body || {};
      ...
      // Save campaign to DB
      const ins = await pool.query(
        `INSERT INTO arb_campaigns (label, kalshi_url, poly_url, kalshi_ticker, auto_enabled, auto_threshold_cents, auto_cooldown_sec, swap_poly, session_id, status, recurring, recurring_type)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'running',$10,$11) RETURNING *`,
        [label || null, kalshiUrl, polyUrl, null, autoEnabled || false, autoThreshold || 3, autoCooldown || 60, swapPoly || false, sessionId, recurring || false, recurringType],
      );
      ...
      if (autoEnabled) {
        startCampaignAuto(campaign.id, sessionId, autoThreshold || 3, autoCooldown || 60, swapPoly || false, recurring || false, recurringType);
      }
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType) {
  ...
  app.patch('/api/arb/campaigns/:id', async (req, res) => {
    try {
      const { id } = req.params;
      const { autoEnabled, autoThreshold, autoCooldown, swapPoly } = req.body;
      ...
      // Update DB
      await pool.query(
        `UPDATE arb_campaigns SET auto_enabled = COALESCE($1, auto_enabled), auto_threshold_cents = COALESCE($2, auto_threshold_cents), auto_cooldown_sec = COALESCE($3, auto_cooldown_sec), swap_poly = COALESCE($4, swap_poly) WHERE id = $5`,
        [autoEnabled, autoThreshold, autoCooldown, swapPoly, id],
      );

      // Start/stop auto
      const existing = campaigns.get(id);
      if (autoEnabled && !existing && campaign.session_id) {
        startCampaignAuto(id, campaign.session_id, autoThreshold || campaign.auto_threshold_cents, autoCooldown || campaign.auto_cooldown_sec, swapPoly ?? campaign.swap_poly);
  ...
  // ── Restore running campaigns on startup ──
  (async () => {
    try {
      ...
      for (const c of r.rows) {
        ...
        try {
          ...
          if (sessionId && c.auto_enabled) {
            startCampaignAuto(c.id, sessionId, c.auto_threshold_cents || 3, c.auto_cooldown_sec || 60, c.swap_poly || false, c.recurring || false, recurringType);
          }
