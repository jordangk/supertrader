...
/** Poly slug unix (UTC) → Kalshi event_ticker fragment e.g. 26MAR301315 (same convention as snipe-mgr). */
...
/**
 * Poly slug epoch and Kalshi KX*15M ticker must agree on the same 15m ET window.
 * e.g. btc-updown-15m-1774893600 → slot 26MAR301400 ↔ KXBTC15M-26MAR301400
 */
function checkCrypto15mKalshiPolyPairing(kalshiTicker, polySlug) {
  ...
  for (const cm of CRYPTO_SLOT_MAP) {
    ...
    const expectedTag = polyUnixToKalshiSlotTag(polyUnix);
...
async function tryCryptoSlotMatch(inputUrl) {
  ...
  for (const cm of CRYPTO_SLOT_MAP) {
    if (isPoly) {
      ...
      const slotTag = polyUnixToKalshiSlotTag(parseInt(timestamp, 10));
      try {
    ...
    if (isKalshi) {
      ...
      let slotUnix = slotTag ? kalshiSlotTagToPolyUnix(slotTag) : null;
      ...
      try {
        ...
        if (slotUnix == null && d.market?.open_time) {
          slotUnix = Math.floor(new Date(d.market.open_time).getTime() / 1000);
        }
      } catch {}
      if (slotUnix == null) continue;
      const slotSec = parseInt(timeframe, 10) * 60;
      slotUnix = Math.floor(slotUnix / slotSec) * slotSec;
      const polySlug = `${cm.asset}-updown-${timeframe}m-${slotUnix}`;
...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  app.post('/api/arb/execute', async (req, res) => {
    try {
      ...
      if (sess2?.bookCache?.data && !marketOrder) {
        ...
        const MIN_DEPTH = 5; // need at least arb size at the ask (was 20 — blocked many Kalshi books)
  ...
  app.post('/api/arb/campaigns', async (req, res) => {
    try {
      ...
      const ins = await pool.query(
        ...
        [label || null, kalshiUrl, polyUrl, null, autoEnabled || false, autoThreshold || 3, autoCooldown || 60, swapPoly || false, sessionId, recurring || false, recurringType, earlyExit || false, exitThreshold || 4, maxSharesSafe],
      );
      ...
      if (autoEnabled) {
        startCampaignAuto(campaign.id, sessionId, autoThreshold || 3, autoCooldown || 60, swapPoly || false, recurring || false, recurringType, earlyExit || false, exitThreshold || 4, maxSharesSafe);
      }
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4, maxShares = GLOBAL_MAX_SHARES) {
    ...
    async function tick() {
      ...
      const pDownBid = swapPoly ? polyUpBid : polyDownBid;
      ...
      if (!state.lastDebug || Date.now() - state.lastDebug > 30000) {
        ...
        const polyBidQtyDbg = swapPoly ? book.poly?.up?.bestBidQty : book.poly?.down?.bestBidQty;
      ...
      // Liquidity: same idea as Arb Lab — both legs need ≥5 shares at the working price or we skip.
      // Top-of-book profit is misleading when KS YES ask is 13¢×1 (you cannot buy 5 @ 13¢).
      const MIN_LIQ = 5;
      const ksYQty = book.kalshi?.yesAskQty;
      const ksNQty = book.kalshi?.noAskQty;
      const polyDn = book.poly?.down;
      const polyUp = book.poly?.up;
      const downLiq = polyDn?.depthAt1c ?? polyDn?.bestAskQty;
      const upLiq = polyUp?.depthAt1c ?? polyUp?.bestAskQty;
      const liqAks = ksYQty;
      const liqApoly = swapPoly ? upLiq : downLiq;
      const liqBks = ksNQty;
      const liqBpoly = swapPoly ? downLiq : upLiq;
      if (bestStrategy === 'A') {
        ...
        if (liqApoly != null && liqApoly < MIN_LIQ) {
          logSkipIfGoodEdge('liquidity', `strat=A poly_down_qty=${liqApoly}<${MIN_LIQ} swapPoly=${swapPoly}`);
          return;
        }
      } else {
        ...
        if (liqBpoly != null && liqBpoly < MIN_LIQ) {
          logSkipIfGoodEdge('liquidity', `strat=B poly_up_qty=${liqBpoly}<${MIN_LIQ} swapPoly=${swapPoly}`);
          return;
        }
      }
      ...
      for (const c of r.rows) {
        ...
        try {
          ...
          if (sessionId && c.auto_enabled) {
            startCampaignAuto(c.id, sessionId, c.auto_threshold_cents || 3, c.auto_cooldown_sec || 60, c.swap_poly || false, c.recurring || false, recurringType, c.early_exit || false, c.exit_threshold_cents || 4, c.max_shares || GLOBAL_MAX_SHARES);
          }
