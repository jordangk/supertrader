...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  app.post('/api/arb/start', async (req, res) => {
    try {
      ...
      }, ARB_BOOK_REFRESH_MS); // configurable book refresh cadence (default 1s)
  ...
  async function startSlotSession(recurringType) {
    ...
    try {
      // Try without status filter — some series don't show as 'open'
      const er = await kalshiFetch(`${KALSHI_TRADE_API}/events?series_ticker=${urls.kalshiSeries}&limit=25`);
      const ed = await er.json();
      const events = ed.events || [];
      const ev = events.find((e) => String(e?.event_ticker || '').toUpperCase().includes(expectedSlotTag))
        || events[0];
      if (!ev) { console.log(`[recurring] No ${urls.kalshiSeries} event matching ${expectedSlotTag}`); return null; }
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4, maxShares = GLOBAL_MAX_SHARES, polyOnlyInput = false) {
    ...
    async function tick() {
      ...
      // ── MAKER KS (ask-1¢ limit 10sh) → wait 500ms → Poly hedge ──
      const entryProfit = bestStrategy === 'A' ? profitA : profitB;
      const KS_SHARES = 10;
      const ksLimitCents = Math.min(99, ksAskPrice + 1); // ask + 1¢ to fill immediately as taker
      ...
      try {
        ...
        if (!clob) {
          ...
          // Step 1: Place KS limit at ask - 1¢ for 10 shares
          const kalshiResult = await placeKalshiOrder(sess.kalshiTicker, ksSide, KS_SHARES, ksLimitCents, { timeInForce: 'immediate_or_cancel' }).catch(e => ({ error: e }));
          ...
          if (ksFilledShares >= 5) {
            ...
            // <$1 value: Poly GTC 5sh @ 99¢
            console.log(`[arb:${campaignId.slice(0,8)}] Poly GTC 5sh @ 99¢ (KS cost \$${kalshiFillCost.toFixed(2)} < \$1)`);
            try {
              const signed = await clob.createOrder({
                tokenID: polyTokenId, price: polyPrice, size: 5, side: 'BUY',
              }, { tickSize: tickSize || '0.01', negRisk: negRisk || false });
              polyResult = await clob.postOrder(signed, 'GTC');
              polyOrderId = polyResult?.orderID;
              polyStatus = polyResult?.status || 'sent';
          ...
          if (!polyFilled) {
            ...
            if (state.polyFailCount >= 3) {
              console.log(
                `[arb-campaign:AUTO-STOP] ${campaignId.slice(0, 8)} — 3 consecutive Polymarket leg failures (IOC not matched / no fill). DB: status=stopped auto_enabled=false. Re-enable via Arb Lab "auto" or PATCH autoEnabled:true after fixing connectivity/book.`,
              );
