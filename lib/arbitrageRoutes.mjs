...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4) {
    ...
    const iv = setInterval(async () => {
      ...
      // DB COOLDOWN CHECK — bulletproof, can't be bypassed by stacked loops
      try {
        const lastTrade = await pool.query(
          `SELECT ts FROM arb_trades WHERE session_id = $1 OR kalshi_ticker LIKE $2 ORDER BY ts DESC LIMIT 1`,
          [state.currentSessionId, '%' + (recurringType || 'NONE').replace('15m', '15M').toUpperCase().replace('BTC', 'BTC').replace('ETH', 'ETH').replace('SOL', 'SOL').replace('XRP', 'XRP').replace('HYPE', 'HYPE').replace('DOGE', 'DOGE').replace('BNB', 'BNB') + '%'],
        );
        if (lastTrade.rows[0]) {
          const secsSinceLast = (Date.now() - new Date(lastTrade.rows[0].ts).getTime()) / 1000;
          if (secsSinceLast < cooldownSec) return; // Too soon — skip
        }
      } catch {}
      ...
      // HARD REJECT: combined cost must be < 96¢ from book data
      if (costA != null && costA > 0.96) { /* profitA already < 2¢, threshold will catch */ }
      if (costB != null && costB > 0.96) { /* same */ }

      // Debug log every 30s
      if (!state.lastDebug || Date.now() - state.lastDebug > 30000) {
        state.lastDebug = Date.now();
        console.log(`[auto-campaign:${campaignId.slice(0,8)}] Book: ksYes=${ksYesAsk} ksNo=${ksNoAsk} polyUp=${polyUpAsk} polyDown=${polyDownAsk} | A=${profitA != null ? (profitA*100).toFixed(1)+'¢' : '—'} B=${profitB != null ? (profitB*100).toFixed(1)+'¢' : '—'} | threshold=${thresholdCents}¢`);
      }

      let bestStrategy = null;
      if (profitA != null && profitA >= threshold && costA <= 0.96 && (profitB == null || profitA >= profitB)) bestStrategy = 'A';
      else if (profitB != null && profitB >= threshold && costB <= 0.96) bestStrategy = 'B';
