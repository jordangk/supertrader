...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  function startCampaignAuto(campaignId, sessionId, thresholdCents, cooldownSec, swapPoly, recurring, recurringType, earlyExit = false, exitThresholdCents = 4) {
    ...
    async function tick() {
      ...
      // Pick whichever side is profitable — no lock, both A and B are valid
      ...
      // Log profitable signals that we skip (no strategy = spread below threshold)
      const bestProfit = Math.max(profitA || -1, profitB || -1);
      if (!bestStrategy && bestProfit > 0) {
        // Profitable but below threshold — log it
        pool.query(
      ...
      // Book depth check removed — just enter, GTC at 99¢ will fill what's available
      ...
      console.log(`[arb:${campaignId.slice(0,8)}] BUY KS ${ksSide.toUpperCase()} 5sh @ ${ksAskPrice}¢ + Poly 5sh @ ${(polyAsk*100).toFixed(0)}¢ = ${combined}¢ profit:${(entryProfit*100).toFixed(1)}¢`);
