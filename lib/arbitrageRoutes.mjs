...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
    ...
    try {
      ...
      for (const [ticker, d] of Object.entries(ks)) {
        ...
        for (const o of d.orders) {
          ...
          const payout = won ? o.shares * 1 : 0;
          const pnl = payout - o.cost - o.fee;
          ksPnl += pnl;
          ksDetail.push({ side: o.side, shares: o.shares, cost: o.cost, payout, pnl, won });
        }
        ...
        if (m) {
          ...
          const utcTs = Math.floor(utcDate.getTime() / 1000);
          // Round to 900s poly slot
          ...
          if (polySlug && poly[polySlug]) {
            ...
            const polyResult = pd.result; // 'up' or 'down'
            for (const t of pd.trades) {
              if (t.type === 'buy') {
                const won = (t.side?.toLowerCase() === polyResult);
                const payout = won ? 5 : 0;
                const pnl = won ? payout - t.cost : -t.cost;
                polyPnl += pnl;
                polyDetail.push({ side: t.side, cost: t.cost, won, pnl });
              } else {
                polyPnl += t.amount;
  ...
  async function startSlotSession(recurringType) {
    ...
    if (!urls) return null;
    const expectedSlotTag = polyUnixToKalshiSlotTag(urls.slot + 15 * 60);
    ...
    try {
      ...
      const mkt = mkts.find((m) => m.status === 'active' && extractKalshiCryptoSlotTag(m.ticker) === expectedSlotTag)
      ...
      if (extractKalshiCryptoSlotTag(kalshiUrl) !== expectedSlotTag) {
