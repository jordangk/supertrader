...
export function registerArbitrageRoutes(app, { getClobClient } = {}) {
  ...
  (async () => {
    ...
    try {
      ...
      // Cancel ALL orphaned resting KS orders from previous server instance
      try {
        ...
        if (oo.ok) {
          ...
          for (const o of orders) {
            await kalshiFetch(`${KALSHI_TRADE_API}/portfolio/orders/${o.order_id}`, { method: 'DELETE' }).catch(() => {});
            console.log(`[arb] Cancelled orphaned resting order ${o.order_id.slice(0,8)} (${o.ticker} ${o.side} ${o.initial_count_fp}sh)`);
          }
          if (orders.length) console.log(`[arb] Cancelled ${orders.length} orphaned resting orders`);
        }
      } catch (e) { console.error('[arb] Orphan cancel error:', e.message?.slice(0, 100)); }
    } catch (e) {
